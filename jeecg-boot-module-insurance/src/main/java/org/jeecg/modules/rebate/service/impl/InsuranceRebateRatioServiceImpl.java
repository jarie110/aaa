...
public class InsuranceRebateRatioServiceImpl extends ServiceImpl<InsuranceRebateRatioMapper, InsuranceRebateRatio> implements IInsuranceRebateRatioService {
   @Resource
    ...
    @Override
    public InsuranceRebateRatio getInsuranceRebateRatioByTypeAndUsageTypeAndInsuranceDate(Integer type, String usageType, Date zbTime,String uid) {
        return rebateRatioMapper.selectInsuranceRebateRatioByTypeAndUsageTypeAndInsuranceDate(type,usageType,zbTime,uid);
    }
     ...
     */
   @Override
    public List<InsuranceRebateRatio> getInsuranceRebateRatioByTypeAndInsuranceDate(Integer type, Date zbTime,String uid) {
        return rebateRatioMapper.selectInsuranceRebateRatioByTypeAndInsuranceDate(type,zbTime,uid);
    }
...
//      使用性质
        ...
        //uid
        String uidList = rebatePojo.getUid();
        String[] uids = uidList.split(",");


        if( rebateType == RebateType.COMMERCIAL_BASIC_REBATE.getType()){
            for (String usage : usages) {
                for (String uid : uids) {
                    //		商业基础险判断
                    //1.根据返点类型和使用性质判断
                    if (usage != null) {
                        InsuranceRebateRatio rebateRatioFromSql = checkRebateIsExists(createTime, rebateType, usage, uid);
...
//                遍历三者险的区间集合
                for (Region region : thirdPartyInsureds) {
                    for (String uid : uids) {
                        if(!carDamageInsured.equals("-") && Integer.parseInt(carDamageInsured) != 0){
                            InsuranceRebateRatio rebateRatioFromSql = rebateRatioMapper.selectInsuranceRebateRatioByTypeAndUsageTypeAndInsuranceDateAndCarDamageInsuredNotZeroAndThirdPartyInsured(rebateType,usage,region.getThirdPartyInsuredStart(),region.getThirdPartyInsuredEnd(),createTime,uid);
                            if (rebateRatioFromSql == null) {//数据库中没有数据
                                InsuranceRebateRatio rebateRatio = new InsuranceRebateRatio();
                                MyBeanUtil.copyPropertiesIgnoreNull(rebatePojo,rebateRatio);
                        ...
                        if(!carDamageInsured.equals("-") && Integer.parseInt(carDamageInsured) == 0){
                            InsuranceRebateRatio rebateRatioFromSql = rebateRatioMapper.selectInsuranceRebateRatioByTypeAndUsageTypeAndInsuranceDateAndCarDamageInsuredZeroAndThirdPartyInsured(rebateType,usage,region.getThirdPartyInsuredStart(),region.getThirdPartyInsuredEnd(),createTime,uid);
                            if (rebateRatioFromSql == null) {//数据库中没有数据
                                InsuranceRebateRatio rebateRatio = new InsuranceRebateRatio();
                                MyBeanUtil.copyPropertiesIgnoreNull(rebatePojo,rebateRatio);
                                rebateRatio.setRebateRatio(new BigDecimal(region.getRebateRatioThird()));//获取返点比例
...
//座位保判断
                if(rebateType == RebateType.SEAT_INSURANCE.getType()){
                    for (String usage : usages) {
                        for (String uid : uids) {
                            InsuranceRebateRatio rebateRatioFromSql = rebateRatioMapper.selectByUsageTypeAndRebateTypeAndInsuranceDate(usage,rebateType,createTime,uid);
                            if (rebateRatioFromSql == null) {//数据库中没有数据
                                InsuranceRebateRatio rebateRatio = new InsuranceRebateRatio();
                                MyBeanUtil.copyPropertiesIgnoreNull(rebatePojo,rebateRatio);
                                rebateRatio.setUsageType(usage);
...
//        竞回返点
        ...
        if(rebateType == RebateType.COMPETITION_REBATE.getType()){
            for (String uid : uids) {
                rebateRatioFromSqls = checkRebateIsExits(createTime, rebateType,uid);
                if (rebateRatioFromSqls.size() == 0) {//数据库中没有数据
                    InsuranceRebateRatio rebateRatio = new InsuranceRebateRatio();
                    MyBeanUtil.copyPropertiesIgnoreNull(rebatePojo,rebateRatio);
                    rebateRatio.setUid(uid);
                    rebateRatioList.add(rebateRatio);
...
//        Integer seatNum = insuranceRebateRatio.getSeatNum();
        ...
        String uid = insuranceRebateRatio.getUid();

//商业基础险
        if(rebateRatioType == RebateType.COMMERCIAL_BASIC_REBATE.getType()){
            InsuranceRebateRatio insuranceRebate = rebateRatioMapper.selectByUsageTypeAndRebateTypeAndInsuranceDate(usageType,rebateRatioType,createTime,uid);
...
//             根据使用性质、返点比、乘客保额、司机保额查询
    ...
    @Override
    public List<InsuranceRebateRatio> getInsuranceRebateRatioByTypeAndCarDamageInsuredIsZeroInsuranceDate(Integer type, String usageType, Date zbTime,String uid) {
        return rebateRatioMapper.selectInsuranceRebateRatioByTypeAndUsageTypeAndInsuranceDateAndCarDamageInsuredZero(type,usageType,zbTime,uid);
    }

    @Override
    public List<InsuranceRebateRatio> getInsuranceRebateRatioByTypeAndCarDamageInsuredIsNotZeroInsuranceDate(Integer type, String usageType, Date zbTime,String uid) {
        return rebateRatioMapper.selectInsuranceRebateRatioByTypeAndUsageTypeAndInsuranceDateAndCarDamageInsuredNotZero(type,usageType,zbTime,uid);
    }

    private List<InsuranceRebateRatio> checkRebateIsExits(Date createTime, Integer rebateType, String uid) {
        List<InsuranceRebateRatio> rebateRatioFromSqls = rebateRatioMapper.selectInsuranceRebateRatioByTypeAndInsuranceDate(rebateType, createTime,uid);
        return rebateRatioFromSqls;
    }

    private InsuranceRebateRatio checkRebateIsExists(Date createTime, Integer rebateType, String usageType,String uid) {
        return rebateRatioMapper.selectInsuranceRebateRatioByTypeAndUsageTypeAndInsuranceDate(rebateType,usageType,createTime,uid);
    }
}
